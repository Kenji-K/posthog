...
class TestFetchSessionEventsActivity:
    ...
    async def test_stashes_lens_llm_inputs_in_redis(self) -> None:
        ...
        # Pydantic serializes inner-row datetimes as `Z`, while session_start_time was already
        # stringified by the activity via `.isoformat()` (`+00:00`).
        assert stored.events == [["$pageview", start.isoformat().replace("+00:00", "Z"), "sess-1"]]
